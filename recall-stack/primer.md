...
Branch: `claude/review-dependencies-2qA0r` (off origin/main `5ec7471`).

Latest (2026-05-22): **claude-seo plugin installed** (this branch) — AgriciDaniel/claude-seo v1.9.9 (6.7k★, MIT) registered as marketplace `agricidaniel-seo` and enabled at project scope via `.claude/settings.json`. Adds 25 sub-skills + 18 sub-agents covering local SEO, Google Maps intelligence (GBP audits, NAP, geo-grid rank tracking), Core Web Vitals via CrUX, Search Console / GA4 hooks, Schema.org validators, SEO drift baselines, and a Playwright visual analyzer. ~3.5k tok always-on overhead. Most directly useful agents for 崑家汽車: `seo-local`, `seo-maps`, `seo-drift`, `seo-schema`, `seo-google`. Loose pre-existing skills (`ai-seo`, `programmatic-seo`, `site-architecture`, `seo-audit`) still load — install is additive, not a replacement.
