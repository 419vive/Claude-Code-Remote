...
function autoDealer(): object {
  return {
    ...
    "sameAs": [
      LINE_OA_URL,
      "https://share.google/EWw99SpeSrkDnqxSt",
...
const HOMEPAGE_FAQS = [
  ...
  { q: "外縣市可以買車嗎？", a: "可以，外縣市客戶搭高鐵來高雄，我們免費到左營高鐵站接您到店看車。" },
...
export async function injectSeoTags(html: string, url: string): Promise<string> {
  ...
  // ---------- Book visit page ----------
    ...
    description = `線上預約到崑家汽車看車，外縣市搭高鐵來高雄，我們免費到左營站接您。高雄在地40年正派經營，看車完全免費無壓力。`;
    jsonLdBlocks.push(breadcrumb([
      { name: "首頁", url: baseUrl },
  ...
  // ---------- Service area pages (/area/:city) ----------
    ...
    if (city) {
      ...
      jsonLdBlocks.push({
        ...
        "offers": {
          "@type": "Offer",
          "description": `${city.name}鄉親搭高鐵來高雄，免費到左營站接送看車`,
          "price": "0",
          "priceCurrency": "TWD",
        },
        "url": canonicalUrl,
      ...
      // ServiceChannel — 左營高鐵站 pickup point for out-of-city visitors
      if (city.type === "out-of-city") {
        jsonLdBlocks.push({
          "@context": "https://schema.org",
          "@type": "ServiceChannel",
          "name": "左營高鐵站免費接送服務",
          "description": `${city.name}鄉親搭高鐵來高雄，我們免費到左營高鐵站接您到店看車，全程輕鬆無壓力。`,
          "serviceType": "免費接駁服務",
          "provider": { "@type": "AutoDealer", "@id": `${baseUrl}/#organization` },
          "availableLanguage": ["zh-TW"],
          "serviceLocation": {
          ...
          "offers": {
            ...
            "price": "0",
            "priceCurrency": "TWD",
            "description": "外縣市客人搭高鐵來高雄，免費接送服務。",
          },
        });
      }
